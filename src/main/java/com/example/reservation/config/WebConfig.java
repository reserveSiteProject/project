...
public class WebConfig implements WebMvcConfigurer {
    ...
    public void addInterceptors(InterceptorRegistry registry) {
        registry.addInterceptor(new LoginCheckInterceptor()) // 인터셉터로 등록할 클래스
                ...
                .addPathPatterns("/admin/*", "/book/*", "/board/review/save", "/board/notice/save", "/reserve", "/myPages/*") // 인터셉터로 체크할 주소(모든주소)
                .excludePathPatterns("/", "/member/save", "/member/login",
                        "/js/**", "/css/**", "/member/image/**", "/image/**",
                        "/*.ico", "/favicon/**", "ws://localhost:8080/ws/chat", "/board"); // 인터셉터 검증을 하지 않을 주소
    }
